...
public class Database {
	...
	//getting the details out for the login page
	public String[][] loginDetails(String records) {
	    ...
	    try {
	        ...
	        while (scan.hasNext()) {
	            String[] lineDetails = scan.nextLine().split("#");
	            detailsList.add(lineDetails);
	        }
	        
	    } catch (Exception e) {
	        errorMessage("Error 104: loginDetails\n" + e.getMessage());
	    }
	    
	    String[][] details = new String[detailsList.size()][];
	    detailsList.toArray(details);
